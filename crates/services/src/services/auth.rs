...
impl AuthContext {
    pub fn new(base_credentials_path: PathBuf) -> Self {
        Self {
            ...
            browser_scoped_auth: parse_bool_env(VIBE_KANBAN_BROWSER_SCOPED_AUTH_ENV)
                .unwrap_or(true),
    ...
    pub async fn get_credentials(&self) -> Option<Credentials> {
        ...
        self.get_credentials_for(current_session_id.as_deref())
            .await
    }
    ...
    pub async fn reload_credentials_for(
        &self,
        session_id: Option<&str>,
    ) -> std::io::Result<Option<Credentials>> {
        let oauth = self.session_state_for(session_id).await.oauth;
        oauth.load().await?;
        Ok(oauth.get().await)
    }
    ...
    pub async fn clear_credentials(&self) -> std::io::Result<()> {
        let current_session_id = self.current_session_id();
        self.clear_credentials_for(current_session_id.as_deref())
            .await
    }
    ...
    pub async fn set_profile(&self, profile: ProfileResponse) {
        let current_session_id = self.current_session_id();
        self.set_profile_for(current_session_id.as_deref(), profile)
            .await;
    }

    pub async fn set_profile_for(&self, session_id: Option<&str>, profile: ProfileResponse) {
        *self
            .session_state_for(session_id)
            .await
            .profile
            .write()
            .await = Some(profile);
